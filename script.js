...
// console.log(content)
	// const res = await page.$$('div:not([class])');
	
	// console.log(res)
	// for (const element of res) {
		// const elementText = await page.evaluate(element => element.innerHTML, element);
		// console.log('Extracted element:', elementText);
		//   }
		// works how i want but need to check if cheerio parsing can return the same
		// let $;
		// res.forEach(async element=>{
			// 	 const elementText = await page.evaluate(element => element.parentElement.innerHTML, element);
			// console.log(typeof elementText,elementText)
			
			//  $ = cheerio.load(elementText)
			//  if($('.team-venue').text()=='Old Trafford'){
...
res.forEach((res,i)=>{
	console.log(`index is ${i+1}`,$(res).html())
})
	// console.log($('div:not([class])').html());
	...
	// );
	// const query = $("matches-component .Matches__fixture").first().html();
// 		const query = $(".matches-component .Matches__fixture").first().html();
// console.log(query)
